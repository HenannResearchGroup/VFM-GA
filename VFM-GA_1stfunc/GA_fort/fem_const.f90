...
!! Nobody should use such a algorithm to fit a neohookean which can be done by hand
    subroutine const_stress_neo(F,theta,stress)
        implicit none
...
&               stress(2,2)
        ...
        dJdF = J*Finv_t
!       http://solidmechanics.org/text/Chapter3_5/Chapter3_5.htm see allan bower's book for more details
...
!! Neohookean gradient wrt each material parameter, for fun only 
    subroutine const_grad_neo(F,theta,grad)
        implicit none
        real(di) F(2,2),F_trans(2,2),B(2,2),I1,J,I1_,dQdI1_,dQdJ, &
&               dI1_dF(2,2),dJdF(2,2),Finv(2,2),Finv_t(2,2),theta(2), &
&               grad(2,4)
...
!!  Plane strain, constitutive law for foam based on spatial log strain measure https://www.sciencedirect.com/science/article/pii/S0022509619303825
    subroutine const_stress_foam(F,theta,stress)
        implicit none
        real(di) F(2,2),E(2,2),K1,devE(2,2),ey(2,2),sumdevE, &
&               K2,N(2,2),Ntemp(3,3),K3,Ntemp_det,J,detF, &
&               G0,B,Jmin,C1,K10,delta_K,X1,X2,C0,theta(nparam), &
...
&       (-(Jmin - 1.d0)/(J - Jmin)) ** r + dlog(J)/J ** r)
    ...
    !! utility function just to make code shorter
    subroutine fill(a,b,c,d,e,f,g,h,i,j,k,l,m,n,theta)
        implicit none
        real(di) a,b,c,d,e,f,g,h,i,j,k,l,m,n,theta(nparam)
        a = theta(1)
        b = theta(2)
        c = theta(3)
        d = theta(4)
    ...
    subroutine const_stress_foam_ps(F,theta,stress)
        implicit none
...
&       G0*(2.d0*X*K2+dLdK2)*N+G0*dLdK3*Y/K2)
    ...
    !! newton raphson solver for plane stress version of the constitutive law 
    subroutine nr_solve_ps(F,theta,F33_guess,S,F_out,F33_out)
        implicit none
        real(di) F(2,2),theta(nparam),F33_guess,F33_next,S(3,3),F_out(3,3),F33_out
        ...
        do while (ite<max_ite.and.S33_ratio.ge.conv_ratio)
            ...
            call const_stress_foam_ps(F1,theta,S1)
            call const_stress_foam_ps(F2,theta,S2)
    ...
    !! this subroutine is only applicable to 1st functionality, when it comes to inhomogeneous field input(2nd functionality), Dacorogna stability check is the only way to go.
    subroutine solve_plane_stress_monok1(F,theta,F_out,Smean,flag,F33_out,S22_out,K1_out)
        ...
        real(di) Smax(3,3),Smin(3,3),Smean(3,3),theta(nparam),F_out(3,3),S33_ratio,S33_good,K1_out
        integer ite1,ite1_max,ite2,ite2_max,flag
        ...
        do while (ite1.le.ite1_max)
            ...
            Fmin(3,3) = F3min
            Fmean(3,3) = F3mean
            call const_stress_foam_ps(Fmax,theta,Smax)
            call const_stress_foam_ps(Fmin,theta,Smin)
            call const_stress_foam_ps(Fmean,theta,Smean)
