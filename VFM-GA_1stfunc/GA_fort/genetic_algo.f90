...
module shinka
    ...
    !note that for speed issue, this only applies to nparam numbers of variables
        implicit none
        integer i,j
    ...
    subroutine fitness(population,coord,disp_mat,connect,rf_mat,fitvalue,cumsump,gen)
        implicit none
        integer(2) population(nt,nbit*nparam),phys_check 
...
&               rf_mat(nnode,nframe*2),disp_mat(nframe,nnode*2)
        ...
        do i = 1,nt
            ...
            if (control_ps == 1) then
                call compute_MSE(coord,disp_mat,connect,rf_mat,obj,base10r)
            elseif (control_ps == 2) then
                if (phys_c_e == 0) then
                    call compute_MSE_ps(coord,disp_mat,connect,rf_mat,obj,base10r,0,0)
                elseif (phys_c_e == 1) then
                    call compute_MSE_ps(coord,disp_mat,connect,rf_mat,obj,base10r,1,0)
                elseif (phys_c_e == 2) then
                    call compute_MSE_ps(coord,disp_mat,connect,rf_mat,obj,base10r,0,1)
                elseif (phys_c_e == 3) then
                    call compute_MSE_ps(coord,disp_mat,connect,rf_mat,obj,base10r,1,1)
                else
    ...
    subroutine fitness_m_ete(population,coord,disp_mat,connect,rf_mat,fitvalue,cumsump,gen,bvi,bpi)
        ! last 4 are best value in and out, best pop in and out from last gen !mother eternal
        implicit none
...
&               rf_mat(nnode,nframe*2),disp_mat(nframe,nnode*2)
    ...
    !! randomly output an integer out of N possible integer
